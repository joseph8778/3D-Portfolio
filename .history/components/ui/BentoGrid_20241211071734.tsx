...
export const BentoGridItem = ({
  ...
  return (
    <div
      className={cn(
        "row-span-1 relative overflow-hidden rounded-3xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none border-transparent justify-between flex flex-col space-y-4",
        className
      )}
      style={{
                ...
                {id === 3 && (
                  <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
                    <div className="flex flex-col gap-3 lg:gap-8 ">
                      {['HTML', 'CSS/Tailwind', 'Javascript', 'React'].map((item) => (
                    ...
                    <div className="flex flex-col gap-3 lg:gap-8 ">
                      <span className="py-4 px-3 rounded-lg text-center bg-[#10132E]" />
                      {['TypeScript', '3JS', 'GSAP', 'NEXT.js'].map((item) => (
                        <span key={item} className="py-2 lg:py4 lg:-px3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                          {item}
                        </span>
                      ))}
